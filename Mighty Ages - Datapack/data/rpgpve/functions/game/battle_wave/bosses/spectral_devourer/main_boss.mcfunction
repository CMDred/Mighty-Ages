...
execute if entity @s[tag=DarkShield,tag=!VoidShield] run particle dust{color:[0d,0d,0d],scale:1} ~ ~ ~ 1.6 1.6 1.6 0.01 60 force
execute if entity @s[tag=!DarkShield,tag=VoidShield] run particle dust{color:[0.541d,0d,0.443d],scale:1} ~ ~ ~ 1.6 1.6 1.6 0.01 60 force

# Shield & Minions
execute if score #BossSpawned Dummy matches 1 run scoreboard players add @s[tag=!VoidShield,tag=!DarkShield,tag=!MakingTriangle,tag=!CastTriangle] Attack1Timer 1
...
execute if score @s Date matches ..0 if score #RNG Dummy matches 1 as @a[tag=IsInRPGPVEGame] run function rpgpve:game/battle_wave/bosses/spectral_devourer/tellraws/ambient/random
#----------------------------------------#
