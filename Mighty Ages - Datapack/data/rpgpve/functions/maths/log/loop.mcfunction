...
execute if score #var LogMath matches 1.. run scoreboard players add out LogMath 100
tellraw @a ["",{"text":"Output = "},{"score":{"name":"out","objective": "LogMath"}}]

scoreboard players remove #LoopAmount LogMath 1
execute if score #var LogMath matches 1.. if score #LoopAmount LogMath matches 1.. run function rpgpve:maths/log/loop