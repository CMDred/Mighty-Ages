...
execute if entity @s[scores={M_PageDelay=0},nbt=!{Items:[{Slot:13b,tag:{MenuItem:1b}}]}] if entity @a[tag=OtherPlayer] run scoreboard players set @s M_NewPage 21
#---------------MENU ITEMS-----------------#
...
tag @s add ThisMenu
execute as @a[tag=OtherPlayer] run loot replace entity @e[type=chest_minecart,tag=ThisMenu,distance=..2,sort=nearest,limit=1] container.13 loot rpgpve:menu/banker/give_gold
execute unless entity @a[tag=OtherPlayer] as @a[tag=ThisPlayer] run loot replace entity @e[type=chest_minecart,tag=ThisMenu,distance=..2,sort=nearest,limit=1] container.13 loot rpgpve:menu/banker/solo

tag @s remove ThisMenu
