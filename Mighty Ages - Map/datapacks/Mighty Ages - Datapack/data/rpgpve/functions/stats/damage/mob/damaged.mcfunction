...
execute if score #UsedProjectile Temp matches 1 run tag @e[type=#minecraft:arrows,sort=nearest,distance=..8,limit=1] add Attacker
#-------------CHANGE HEALTH--------------#
execute if score #UsedProjectile Temp matches 1 on attacker at @s run playsound minecraft:entity.experience_orb.pickup master @s ~ ~ ~ 0.6 1
#-----------CALCULATE DAMAGE-------------#
function rpgpve:stats/calculate/damages
#-------------REMOVE HEALTH--------------#
function rpgpve:stats/damage/mob/remove_health
#-------------DAMAGE RECORD--------------#
execute unless score #UsedProjectile Temp matches 1 on attacker run function rpgpve:game/records/highest_melee/check
#--------------UPDATE BAR----------------#
